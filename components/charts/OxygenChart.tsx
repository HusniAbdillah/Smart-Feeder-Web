...
const LINES = [{ key: "dissolvedOxygen", label: "Oksigen Terlarut (ppm)", color: COLORS.oxygen }];

export function OxygenChart({ initialData }: { initialData?: SmartFeederSensorData[] }) {
      ...
      {LINES.map((line) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-xs font-semibold text-text-main">{line.label}</span>
          </button>
        );
      })}
    </div>
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={chartData} margin={{ top: 8, right: 16, left: -20, bottom: 0 }}>
            ...
            />
            <Legend content={<OxygenLegend />} />

            <ReferenceLine y={4} stroke="#0EA5E9" strokeDasharray="3 3" label={{ position: "insideTopLeft", value: "Ideal 4 ppm", fill: "#0EA5E9", fontSize: 10 }} />
            <ReferenceLine y={6} stroke="#0EA5E9" strokeDasharray="3 3" label={{ position: "insideBottomLeft", value: "Ideal 6 ppm", fill: "#0EA5E9", fontSize: 10 }} />

            <Line
              type="monotone"
              dataKey="dissolvedOxygen"
              name="Oksigen Terlarut (ppm)"
              stroke={COLORS.oxygen}
              strokeWidth={3}
              dot={false}
              hide={!visibleLines.dissolvedOxygen}
              activeDot={{ r: 6, strokeWidth: 0, fill: COLORS.oxygen }}
            />
          </LineChart>
        </ResponsiveContainer>
      )}
